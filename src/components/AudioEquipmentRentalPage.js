...
// Default image for audio equipment suppliers
const DEFAULT_AUDIO_IMAGE = '/images/venues/8.png';
...
const AudioEquipmentRentalPage = () => {
  ...
  // Fetch audio equipment suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Audio Equipment Rental')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching audio equipment suppliers:', error);
        setError('Failed to load audio equipment suppliers');
        return;
      }
      ...
      const transformedData = data.map(supplier => ({
        id: supplier.id,
        name: supplier.company_name || supplier.full_name || 'Audio Equipment Provider',
      ...
      console.log('Fetched audio equipment suppliers:', transformedData);
    } catch (err) {
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Audio Equipment Rental Services</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading audio equipment providers...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No audio equipment providers registered yet.' : 'No audio equipment providers found matching your search.'}
          </div>
        ) : (
