...
// Default image for balloon artist suppliers
const DEFAULT_BALLOON_IMAGE = '/images/venues/18.png';
...
const BalloonArtistsPage = () => {
  ...
  // Fetch balloon artist suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Balloon Artists')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching balloon artist suppliers:', error);
        setError('Failed to load balloon artist suppliers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Balloon Artists</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading balloon artists...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No balloon artists registered yet.' : 'No balloon artists found matching your search.'}
          </div>
        ) : (
