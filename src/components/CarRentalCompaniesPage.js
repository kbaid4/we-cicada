...
// Default image for car rental companies
const DEFAULT_CAR_IMAGE = '/images/venues/10.png';
...
const CarRentalCompaniesPage = () => {
  ...
  // Fetch car rental companies from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Car Rental Companies')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching car rental companies:', error);
        setError('Failed to load car rental companies');
        return;
      }
      ...
      const transformedData = data.map(supplier => ({
        id: supplier.id,
        name: supplier.company_name || supplier.full_name || 'Car Rental Company',
      ...
      console.log('Fetched car rental companies:', transformedData);
    } catch (err) {
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Car Rental Companies</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading car rental companies...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No car rental companies registered yet.' : 'No car rental companies found matching your search.'}
          </div>
        ) : (
