...
// Default image for chauffeur services
const DEFAULT_CHAUFFEUR_IMAGE = '/images/venues/11.png';
...
const ChauffeurServicesPage = () => {
  ...
  // Fetch chauffeur services from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Chauffeur Services')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching chauffeur services:', error);
        setError('Failed to load chauffeur services');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Chauffeur Services</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading chauffeur services...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No chauffeur services registered yet.' : 'No chauffeur services found matching your search.'}
          </div>
        ) : (
