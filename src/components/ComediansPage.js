...
// Default image for comedian suppliers
const DEFAULT_COMEDIAN_IMAGE = '/images/venues/4.png';
...
const ComediansPage = () => {
  ...
  // Fetch comedian suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Comedians')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching comedian suppliers:', error);
        setError('Failed to load comedian suppliers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Comedians</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading comedians...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No comedians registered yet.' : 'No comedians found matching your search.'}
          </div>
        ) : (
