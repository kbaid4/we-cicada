...
// Default image for content creation service suppliers
const DEFAULT_CONTENT_IMAGE = '/images/venues/15.png';
...
const ContentCreationServicesPage = () => {
  ...
  // Fetch content creation service suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Content Creation Services')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching content creation services:', error);
        setError('Failed to load content creation services');
        return;
      }
      ...
      const transformedData = data.map(supplier => ({
        id: supplier.id,
        name: supplier.company_name || supplier.full_name || 'Content Creation Service',
      ...
      console.log('Fetched content creation services:', transformedData);
    } catch (err) {
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Content Creation Services</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading content creation services...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No content creation services registered yet.' : 'No content creation services found matching your search.'}
          </div>
        ) : (
