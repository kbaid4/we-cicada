...
// Default image for corporate catering suppliers
const DEFAULT_CATERING_IMAGE = '/images/venues/15.png';
...
const CorporateCateringPage = () => {
  ...
  // Fetch corporate catering suppliers from Supabase
  useEffect(() => {
    fetchCaterers();
  }, []);
  ...
  const fetchCaterers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Corporate Catering Services')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching corporate caterers:', error);
        setError('Failed to load corporate caterers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Corporate Catering Services</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading corporate caterers...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {caterers.length === 0 ? 'No corporate caterers registered yet.' : 'No corporate caterers found matching your search.'}
          </div>
        ) : (
