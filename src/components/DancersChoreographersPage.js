...
// Default image for dancer suppliers
const DEFAULT_DANCER_IMAGE = '/images/venues/6.png';
...
const DancersChoreographersPage = () => {
  ...
  // Fetch dancer suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Dancers/Choreographers')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching dancer suppliers:', error);
        setError('Failed to load dancer suppliers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Dancers & Choreographers</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading dancers...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No dancers registered yet.' : 'No dancers found matching your search.'}
          </div>
        ) : (
