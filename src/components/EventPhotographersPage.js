...
// Default image for event photographers
const DEFAULT_PHOTOGRAPHER_IMAGE = '/images/venues/1.png';
...
const EventPhotographersPage = () => {
  ...
  // Fetch event photographers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Event Photographers')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching event photographers:', error);
        setError('Failed to load event photographers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Event Photographers</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading event photographers...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No event photographers registered yet.' : 'No event photographers found matching your search.'}
          </div>
        ) : (
