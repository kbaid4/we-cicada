      ...
      try {
        ...
        for (const invite of invites) {
          ...
          if (!profilesError && profiles && profiles.length > 0) {
            // Use profile information if available - supplier has signed in
            supplierDetails.push({
              email: invite.supplier_email,
              name: profiles[0].full_name || profiles[0].company_name || invite.supplier_email,
              serviceType: profiles[0].serviceType || '',
              status: 'Accepted' // Supplier has signed in
            });
          } else {
            // Supplier hasn't signed in yet - show pending status
            supplierDetails.push({
              email: invite.supplier_email,
