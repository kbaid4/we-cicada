...
// Default image for florist suppliers
const DEFAULT_FLORIST_IMAGE = '/images/venues/15.png';
...
const FloristsPage = () => {
  ...
  // Fetch florist suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Florists')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching florist suppliers:', error);
        setError('Failed to load florist suppliers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Florists</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading florists...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No florists registered yet.' : 'No florists found matching your search.'}
          </div>
        ) : (
