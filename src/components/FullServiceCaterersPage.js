...
// Default image for full-service caterer suppliers
const DEFAULT_CATERER_IMAGE = '/images/venues/9.png';
...
const FullServiceCaterersPage = () => {
  ...
  // Fetch full-service caterer suppliers from Supabase
  useEffect(() => {
    fetchCaterers();
  }, []);
  ...
  const fetchCaterers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Full-Service Caterers')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching full-service caterers:', error);
        setError('Failed to load full-service caterers');
        return;
      }
      ...
      const transformedData = data.map(supplier => ({
        id: supplier.id,
        name: supplier.company_name || supplier.full_name || 'Full-Service Caterer',
      ...
      console.log('Fetched full-service caterers:', transformedData);
    } catch (err) {
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Full-Service Caterers</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading full-service caterers...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {caterers.length === 0 ? 'No full-service caterers registered yet.' : 'No full-service caterers found matching your search.'}
          </div>
        ) : (
