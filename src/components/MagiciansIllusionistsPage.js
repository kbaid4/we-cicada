...
// Default image for magician suppliers
const DEFAULT_MAGICIAN_IMAGE = '/images/venues/5.png';
...
const MagiciansIllusionistsPage = () => {
  ...
  // Fetch magician suppliers from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Magicians/Illusionists')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching magician suppliers:', error);
        setError('Failed to load magician suppliers');
        return;
      }
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Magicians & Illusionists</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading magicians...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No magicians registered yet.' : 'No magicians found matching your search.'}
          </div>
        ) : (
