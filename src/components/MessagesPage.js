...
const MessagesPage = () => {
    ...
    const fetchEventsAndSuppliers = async () => {
      try {
        ...
        if (user.isSupplier) {
          ...
          // Also add direct connection conversations
          const { data: connections } = await supabase
            .from('messages')
            .select('admin_id, supplier_email')
            .eq('admin_id', adminId)
            .is('event_id', null)
          ...
          if (connections && connections.length > 0) {
            ...
            const uniqueConnections = connections.reduce((acc, conn) => {
              const key = `${conn.admin_id}_${conn.supplier_email}`;
              if (!acc[key]) {
                acc[key] = conn;
              }
              return acc;
            ...
            const connectionPairs = Object.values(uniqueConnections).map(conn => ({
              eventId: null, // No event for direct connections
              eventName: 'Direct Connection',
              supplierName: conn.supplier_email,
              supplierEmail: conn.supplier_email,
              isDirectConnection: true
            }));
            
            pairs = [...pairs, ...connectionPairs];
          }
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      // Handle direct connections vs event-based conversations
      if (selectedPair.isDirectConnection) {
        // For direct connections, filter by admin_id and supplier_email with no event_id
        query = query
          .is('event_id', null)
          .eq('admin_id', adminId)
          .eq('supplier_email', selectedPair.supplierEmail);
      } else if (selectedPair.eventId) {
        // For event-based conversations
        query = query.eq('event_id', selectedPair.eventId);
      } else {
        console.log('Invalid conversation type, skipping message fetch');
        return;
      }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const newMessage = {
        event_id: selectedPair?.isDirectConnection ? null : selectedPair?.eventId,
