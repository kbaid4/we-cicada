...
const SupplierMessagesPage = () => {
  ...
  const fetchMessages = useCallback(async (eventId, supplierEmail) => {
    try {
      ...
      if (eventId === null) {
        // Handle direct connection conversations
        query = query
          .is('event_id', null)
          .eq('supplier_email', supplierEmail);
      } else {
        // Handle event-based conversations
        query = query
          .eq('event_id', eventId)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Also check for direct connection conversations
        ...
        if (directMessages && directMessages.length > 0) {
          // Get unique admin connections
          const uniqueAdmins = [...new Set(directMessages.map(m => m.admin_id))].filter(Boolean);
          
          // Add direct connection entries
          const directConnections = uniqueAdmins.map(adminId => ({
          ...
          relevantEvents.push(...directConnections);
        }
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const notificationData = {
        ...
        type: 'new_message',
        content: `New message from supplier: ${trimmedContent.substring(0, 50)}${trimmedContent.length > 50 ? '...' : ''}`,
