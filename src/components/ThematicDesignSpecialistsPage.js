...
// Default image for thematic design specialists
const DEFAULT_THEME_IMAGE = '/images/venues/17.png';
...
const ThematicDesignSpecialistsPage = () => {
  ...
  // Fetch thematic design specialists from Supabase
  useEffect(() => {
    fetchSuppliers();
  }, []);
  ...
  const fetchSuppliers = async () => {
    try {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('*')
        .eq('service_type', 'Thematic Design Specialists')
        .eq('user_type', 'supplier');

      if (error) {
        console.error('Error fetching thematic design specialists:', error);
        setError('Failed to load thematic design specialists');
        return;
      }
      ...
      const transformedData = data.map(supplier => ({
        id: supplier.id,
        name: supplier.company_name || supplier.full_name || 'Thematic Design Specialist',
      ...
      console.log('Fetched thematic design specialists:', transformedData);
    } catch (err) {
  ...
  return (
    <div className={styles['app-container']}>
      ...
      <div className={styles['welcome-section']}>
        <h1 className={styles['welcome-text']}>Thematic Design Specialists</h1>
      </div>
      ...
      <div className={styles['hotels-grid']}>
        {loading ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>Loading thematic design specialists...</div>
        ) : error ? (
          <div style={{ color: '#d32f2f', fontWeight: 500, fontSize: 18, marginTop: 40 }}>{error}</div>
        ) : filtered.length === 0 ? (
          <div style={{ color: '#441752', fontWeight: 500, fontSize: 18, marginTop: 40 }}>
            {suppliers.length === 0 ? 'No thematic design specialists registered yet.' : 'No thematic design specialists found matching your search.'}
          </div>
        ) : (
